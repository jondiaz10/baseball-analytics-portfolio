...
def run(start_date: str, end_date: str) -> None:
    ...
    if df.empty:
        ...
        _print_summary(start_date, end_date, rows_extracted, gcs_uri=None, rows_loaded=0)
        return

    gcs_uri = upload_to_gcs(df, bucket, start_date, end_date)
    rows_loaded = load_to_bigquery(gcs_uri, bq_project, bq_dataset, bq_table)

    _print_summary(start_date, end_date, rows_extracted, gcs_uri, rows_loaded)
